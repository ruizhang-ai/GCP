...
class DateFinder(object):
    ...
    def extract_date_strings(self, text, strict=False):
        ...
        for match in self.DATE_REGEX.finditer(text):
            ...
            digits_modifier = captures.get('digits_modifier')
            ...
            if len(digits) == 1 and len(digits[0]) != 4:
                continue
            if len(digits) == 2 and len(months) ==0:
                continue
            if len(digits_modifiers) > 0:
                if len(months)==1 and len(digits)==1:
                    continue
            ...
            ## 19 February 2013 year 09:10
                ...
                complete = True
            elif (len(months) == 1) and (len(digits) == 1) and (len(digits_modifier)==1):
                complete = True
            ...
            patterns = 'due|by|on|standard|daylight|savings|time|date|of|to|until|z|at|t'.split('|')
            match_str = remove_endswith(match_str, patterns)
            match_str = remove_startswith(match_str, patterns)
            #print match_str

            ## Save sanitized source string
            yield match_str, indices, captures

def remove_endswith(sentence, patterns):
    for pattern in patterns:
        if sentence.endswith(((' '+pattern), (' '+pattern.title()))):
            return sentence[:-len(pattern)].strip()
    return sentence.strip()
def remove_startswith(sentence, patterns):
    for pattern in patterns:
        if sentence.startswith(((pattern+' '),((pattern.title()+' ')))):
            return sentence[len(pattern):].strip()
    return sentence.strip()
