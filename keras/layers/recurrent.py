...
class GLSTMCell(Layer):
    def __init__(self, units,
                 ...
                 implementation=2,
                 **kwargs):
        super(GLSTMCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def call(self, inputs, states, training=None):
        ...
        inputs1 = inputs[:,:input_dim]
        inputs2 = inputs[:,input_dim:]
        ...
        if self.implementation == 1:
            if 0 < self.dropout < 1.:
                inputs_i_ent = inputs2 * dp_mask[0][:,input_dim:]
                inputs_f_ent = inputs2 * dp_mask[1][:,input_dim:]
                inputs_c_ent = inputs2 * dp_mask[2][:,input_dim:]
                inputs_o_ent = inputs2 * dp_mask[3][:,input_dim:]
            else:
                inputs_i_ent = inputs2
                inputs_f_ent = inputs2
                inputs_c_ent = inputs2
                inputs_o_ent = inputs2
            x_i_ent = K.dot(inputs_i_ent, self.kernel_i)
            x_f_ent = K.dot(inputs_f_ent, self.kernel_f)
            x_c_ent = K.dot(inputs_c_ent, self.kernel_c)
            x_o_ent = K.dot(inputs_o_ent, self.kernel_o)
            if self.use_bias:
                x_i_ent = K.bias_add(x_i_ent, self.bias_i)
                x_f_ent = K.bias_add(x_f_ent, self.bias_f)
                x_c_ent = K.bias_add(x_c_ent, self.bias_c)
                x_o_ent = K.bias_add(x_o_ent, self.bias_o)

            if 0 < self.recurrent_dropout < 1.:
                h_tm1_i_ent = h_tm1 * rec_dp_mask[0]
                h_tm1_f_ent = h_tm1 * rec_dp_mask[1]
                h_tm1_c_ent = h_tm1 * rec_dp_mask[2]
                h_tm1_o_ent = h_tm1 * rec_dp_mask[3]
            else:
                h_tm1_i_ent = h_tm1
                h_tm1_f_ent = h_tm1
                h_tm1_c_ent = h_tm1
                h_tm1_o_ent = h_tm1
            i_ent = self.recurrent_activation(x_i_ent + K.dot(h_tm1_i_ent,self.recurrent_kernel_i))
            f_ent = self.recurrent_activation(x_f_ent + K.dot(h_tm1_f_ent,self.recurrent_kernel_f))
            c_ent = (f_ent * c_tm1) + (i_ent * self.activation(x_c_ent + K.dot(h_tm1_c_ent,self.recurrent_kernel_c))) 
            o_ent = self.recurrent_activation(x_o_ent + K.dot(h_tm1_o_ent,self.recurrent_kernel_o))
            h_ent = o_ent * self.activation(c_ent)

            if 0 < self.dropout < 1.:
                inputs_i = inputs1 * dp_mask[0][:,:input_dim]
                inputs_f = inputs1 * dp_mask[1][:,:input_dim]
                inputs_c = inputs1 * dp_mask[2][:,:input_dim]
                inputs_o = inputs1 * dp_mask[3][:,:input_dim]
            else:
                inputs_i = inputs1
                inputs_f = inputs1
                inputs_c = inputs1
            ...
            if 0 < self.recurrent_dropout < 1.:
                h_tm1_i = h_ent * rec_dp_mask[0]
                h_tm1_f = h_ent * rec_dp_mask[1]
                h_tm1_c = h_ent * rec_dp_mask[2]
                h_tm1_o = h_ent * rec_dp_mask[3]
            else:
                h_tm1_i = h_ent
                h_tm1_f = h_ent
                h_tm1_c = h_ent
            ...
            c = (f * c_ent) + (i * self.activation(x_c + K.dot(h_tm1_c,self.recurrent_kernel_c)))
...
class GLSTM(RNN):
    ...
    def __init__(self, units,
        ...
        cell = GLSTMCell(units,
                        ...
                        recurrent_dropout=recurrent_dropout,
                        implementation=implementation)
        super(GLSTM, self).__init__(cell,
                                   return_sequences=return_sequences,
                                   return_state=return_state,
                                   go_backwards=go_backwards,
                                   stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._generate_dropout_mask(inputs, training=training)
        self.cell._generate_recurrent_dropout_mask(inputs, training=training)
        return super(GLSTM, self).call(inputs,
                                      mask=mask,
                                      training=training,
                                      initial_state=initial_state)
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(GLSTM, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
